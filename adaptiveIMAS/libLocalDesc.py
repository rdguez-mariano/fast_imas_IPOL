...
import subprocess
import pandas
def IMAScaller(img1,img2, desc = 11, AdOPT = None, covering=-1.0, GFilter='IntCode-2', Visual=False, EuPrecision=24, Detector='HessAff', dilate=False):    
    ...
    _ = subprocess.check_output("cp imasbuild/z_main /tmp", shell=True)
    ...
    if AdOPT is not None:
        ...
        _ = subprocess.call("rm /tmp/2simu.csv", shell=True)
        _ = subprocess.call("rm /tmp/2simu2.csv", shell=True)
        
        np.savetxt('/tmp/2simu.csv', (t1,r1), delimiter=',')
        np.savetxt('/tmp/2simu2.csv', (t2,r2), delimiter=',')
        _ = subprocess.check_output("cat /tmp/2simu2.csv >> /tmp/2simu.csv", shell=True)       
        _ = subprocess.check_output('cd /tmp && '+cmdflag+ ' ./z_main -im1 "./img1.png" -im2 "./img2.png" -desc %d -covering -0.5 -applyfilter %d > imas.out'%(desc,GFilterCode), shell=True)
    else:
        _ = subprocess.check_output('cd /tmp && '+cmdflag+ ' ./z_main -im1 "./img1.png" -im2 "./img2.png" -desc %d -applyfilter %d -covering %f > imas.out'%(desc,GFilterCode,covering), shell=True)
    ...
    Nsimus1 = int(subprocess.check_output('cd /tmp && cat imas.out | grep "image 1" | cut -f 14 -d" " ', shell=True).decode('utf-8'))
    Nsimus2 = int(subprocess.check_output('cd /tmp && cat imas.out | grep "image 2" | cut -f 14 -d" " ', shell=True).decode('utf-8'))
    ...
    Ntotal = int(subprocess.check_output('cd /tmp && cat imas.out | grep "possible matches have been found" | cut -f 4 -d" " ', shell=True).decode('utf-8'))
    if GFilterCode>0:
